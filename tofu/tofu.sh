...
TF_VAR_proxmox_api_token="$(op item get "Proxmox OpenTofu API Token" --vault "$OP_VAULT_ID" --fields token_id --format json | jq -r .value)=$(op item get "Proxmox OpenTofu API Token" --vault "$OP_VAULT_ID" --fields credential --format json | jq -r .value)"
